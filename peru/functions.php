...
//Buscar codigo de ciudad
function Search_code_city($name, $country)
{
	$dbHost = '104.130.46.73';
	$dbName = 'nikkenla_incorporation';
	$dbUser = 'nikkenla_mkrt';
	...
	try
	{
		...
		$queryResult = $pdo->prepare("SELECT code FROM nikkenla_incorporation.citys where name = :name and country = :country");
		$queryResult->execute(array(':name' => $name, ':country' => $country));
		$done = $queryResult->fetch();
		if($done)
		{
			return $done['code'];
