...
$second_last_name = strtoupper($_POST["second_last_name"]);
...
$name_separate=$last_name."-".$second_last_name."|".$name;

//echo $second_last_name;
//echo $name_separate;
//exit;

if($last_name != "" || $second_last_name != ""){	$name = $last_name ." ". $second_last_name . ", " . $name;	}
...
if (isset($_SESSION["sponsor"]) && isset($_SESSION["kit"]) && isset($_SESSION["codeticket"])) {
...
if($done)
{
	echo 'el <strong>codigo'.$idticket.' ya se encuentra redimido</strong>, por favor utilizar uno nuevo';
	exit;
	/*if($done['payment'] == 0)
	{
...
if($done)
{
	echo 'el <strong>codigo'.$code.' ya se encuentra utilizado en la iw</strong>, por favor utilizar uno nuevo';
...

{
	...
	$sql = "INSERT INTO contracts_test (id_contract, country, code, name, type, type_incorporate, type_sponsor, sponsor, email, cellular, birthday, address, residency_one, residency_two, residency_three, residency_four, name_legal_representative, type_document, number_document, name_cotitular, type_document_cotitular, number_document_cotitular, bank, bank_type, number_account, number_clabe, rfc, ip, browser, gender, kit, playera, talla, verify_digit, separate_name) VALUES (:id, :country, :code, :name, :type, :type_incorporate, :type_sponsor, :sponsor, :email, :cellular, :birthday, :address, :residency_one, :residency_two, :residency_three, :residency_four, :name_legal_representative, :type_document, :number_document, :name_cotitular, :type_document_cotitular, :number_document_cotitular, :bank, :bank_type, :number_account, :number_clabe, :rfc, :ip, :browser, :gender, :kit, :playera, :tallaLetra, :verify_digit, :separate_name)";
