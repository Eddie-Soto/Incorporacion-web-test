...
//Informaci�n por pa�s
	function Search_country_info($country){
		if($country == 0)
		{
			return array("servicio.lat@nikkenlatam.com", "latinoamerica", "Latinoam�rica", "LAT");
		}
		if($country == 1)
		{
		...
		if($country == 2)
		{
			return array("servicio.mex@nikkenlatam.com", "mexico", "M�xico", "MEX");
		}
		if($country == 3)
		{
			return array("servicio.per@nikkenlatam.com", "peru", "Per�", "PER");
		}
		if($country == 4)
		{
		...
		if($country == 5)
		{
			return array("servicio.pan@nikkenlatam.com", "panama", "Panam�", "PAN");
		}
		if($country == 6)
		{
	...
	//Asignar cliente de UBI
		$queryResult = $pdo->prepare("SELECT code as code_customers, name as name_customers, email as email_customers, phone as phone_customers, id_catalog_countrie as id_catalog_countrie_customer FROM nikkenla_ubi.customers where email = :email and status = 1 ORDER BY create_at DESC LIMIT 1");
	  	...
	  	if($row > 0){
	  		...
	  		/*Validar si el asesor est� como nivel 0 -> Manual */
			  	...
			  	if($row > 0){
			  		...
			  		/*Guardar en base de datos*/
						...
						$result = $query->execute([
							'code' => $code_customers,
							'level' => 0,
							'user_create' => 'Proceso autom�tico UBI'
						]);
					/*Guardar en base de datos*/

					/*Actualizar informaci�n del nuevo registro*/
			...
			/*Guardar registro cliente manual*/
				...
				$result = $query->execute([
					...
					'user_assigned' => 'Proceso autom�tico UBI',
