...
/*vars*/
$acountbank=$_POST['acountbank'];

$country = $_POST["country"];
$bankvalidate = $_POST["choose_banks_dates"];
$type = $_POST["type"];
...
<ul>
	...
	if($type == 1)
	{
		...
		//}
		if($country==5 and $acountbank==1) {
				$concat = $concat . "<li>Anexar certificado bancario y/o encabezado del estado de cuenta bancario.<small>(debe detallarse tipo de cuenta, nombre del banco, cuenta bancaria, nombre del titular de la cuenta, de no anexar las imagenes no se guardará en esta afiliación).</small></li>";
				$counter_images++;
			}
	...
	if($type_incorporate == 0)
	{
		...
		if($country == 5)
		{
			...
			$concat = $concat . "<li>Anexar copia de identificación vigente del Representante Legal ( Ambos lados)</li>";
...
<script>
	...
	// enable fileuploader plugin
		$('input[name="files"]').fileuploader({
	        ...
	        theme: 'dragdrop',
			upload: {
	            url: 'plugins/upload/examples/drag-drop/php/subir_ajax_file.php',
	            data: null,
	            type: 'POST',
	            ...
	            onProgress: function(data, item) {
	                ...
	                if(progressBar.length > 0) {
	                 ...
	                 //document.getElementById("column-actions").setAttribute("hidden","true");
	            },
	            onComplete: null,
	        },
